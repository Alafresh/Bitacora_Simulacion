...
function patchworkGenerator(numTypes) {
  const p = 0.35,
    ...
    SELF = 0.0
  const rnd = (i, j) =>
    Math.abs((Math.sin((i * 73856093) ^ (j * 19349663)) * 43758.5453) % 1)
  const m = []
  for (let i = 0; i < numTypes; i++) {
    m.push([])
    for (let j = 0; j < numTypes; j++) {
...
export function getForcePreset(id, numTypes) {
  ...
  for (let i = 0; i < numTypes; i++) {
    for (let j = 0; j < numTypes; j++) {
      const value =
        finalMatrix[i] && typeof finalMatrix[i][j] === 'number'
          ? finalMatrix[i][j]
          : 0
