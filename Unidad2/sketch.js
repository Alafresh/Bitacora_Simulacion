...
window.draw = function () {
  ...
  if (
    ...
    (joyVec.magSq() > 0 || isBtnPressed || increaseMass || decreaseMass)
  ) {
    audioManager.init()
  }
  ...
  if (attractor.isAwake) {
    for (let j = stardust.length - 1; j >= 0; j--) {
      let p = stardust[j]
      let distSq =
        (p.pos.x - attractor.pos.x) ** 2 + (p.pos.y - attractor.pos.y) ** 2
      let captureRadius = 5
      ...
      if (distSq < captureRadius * captureRadius) {
        ...
        capturedParticle.orbitRadius = random(
          visualSize * 0.7,
          visualSize * 2.2,
        )
...
function changeRandomPreset() {
  ...
  console.log(
    `[Campos Invisibles] Ecosistema mutado. Preset ID: ${nextPresetId}`,
  )
}

function explodeParticles(attractor, isOverload, currentEncRaw) {
  let amountToShoot = isOverload
    ? attractor.orbitingParticles.length
    : Math.min(15, attractor.orbitingParticles.length)
